...
<div class="group relative overflow-hidden rounded-2xl border bg-white hover:shadow-sm transition">

    {{-- 📌 Pinned badge --}}
    @if($isPinned)
        <span class="absolute top-2 left-2 z-10 text-xs px-2 py-1 rounded-full
    ...
    @if($canPin)
        <form method="POST"
            ...
            @csrf
            <button type="submit"
                    class="text-xs px-2 py-1 rounded-full border bg-white hover:bg-gray-50 shadow-sm">
                {{ $isPinned ? 'Unpin' : 'Pin' }}
            </button>
        </form>
    @endif
                ...
                <img
                    ...
                    onerror="
                        if (this.dataset.fallback && this.src !== this.dataset.fallback) {
                            this.src = this.dataset.fallback; return;
                        }
                        this.onerror=null;
                        this.closest('div').innerHTML =
                          '<div class=&quot;h-full w-full flex items-center justify-center text-xs text-gray-500&quot;>Image unavailable</div>';
                    "
                    data-fallback="{{ $full ?? '' }}"
                >
            @else
